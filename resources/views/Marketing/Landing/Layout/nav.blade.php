    ...
    <div class="bg-blue-600 relative">
        <div class="flex justify-between items-center max-w-7xl mx-auto border-b-2 border-gray-100 px-4 py-6 sm:px-6 md:justify-start md:space-x-10 lg:px-8">
            ...
            @foreach($nav['navigation'] as $item)
                <a href="{{ $item['route'] }}" class="text-base font-medium text-blue-100 hover:text-white"> {{ $item['routeName'] }} </a>
            @endforeach
            </nav>
            <div class="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
                <a href="{{ $nav['loginRoute'] }}" class="whitespace-nowrap text-base font-medium text-blue-100 hover:text-white"> {{ $nav['loginName'] }} </a>
                <a href="{{ $nav['registerRoute'] }}" class="ml-8 whitespace-nowrap inline-flex items-center justify-center bg-blue-800 px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white hover:bg-blue-900"> {{ $nav['registerName'] }} </a>
            </div>
        </div>
                ...
                <div class="pt-5 pb-6 px-5">
                    ...
                    <div class="mt-4 border-t-2 border-gray-100">
                        <nav class="mt-10 grid grid-cols-1 gap-7">
                            @foreach($nav['navigation'] as $item)
                            <a href="{{ $item['route'] }}" class="p-3 flex items-center rounded-lg hover:bg-gray-50 ml-4 text-base font-medium text-gray-900">
                                {{ $item['routeName'] }}
                            </a>
                            @endforeach
                        </nav>
